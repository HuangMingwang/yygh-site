pipeline {
    ...
    stages {
        stage('拉取代码') {
            ...
            steps {
                container('nodejs') {
                    git(url: 'https://github.com/HuangMingwang/yygh-site.git', credentialsId: 'github-id', branch: 'master', changelog: true, poll: false)
                    sh 'ls -al'
                }
        ...
        stage('发送确认邮件') {
            agent none
            steps {
                mail(to: '1125385880@163.com', subject: 'yygh-site构建结果', body: "构建成功了  $BUILD_NUMBER")
            }
        }
